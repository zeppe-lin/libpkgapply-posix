...
if grep -F 'O_RDONLY | cloexec_flag() | nofollow_flag()));' \
     "$root/src/checkpoint_store.cpp" "$root/src/completed_evidence_store.cpp" \
...
grep -F 'O_RDONLY | cloexec_flag() | nofollow_flag() | O_NONBLOCK' \
  "$root/src/journal_store.cpp" >/dev/null || fail 'journal snapshot open is blocking'
grep -F 'O_NONBLOCK' "$root/src/checkpoint_store.cpp" >/dev/null || fail 'checkpoint snapshot open is blocking'
