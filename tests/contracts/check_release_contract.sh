...
grep -F "version: '3.2.3'" "$root/meson.build" >/dev/null || fail 'project version mismatch'
grep -F 'PROJECT_NUMBER         = 3.2.3' "$root/Doxyfile" >/dev/null || fail 'Doxygen version mismatch'
grep -F 'return "3.2.3";' "$root/src/version.cpp" >/dev/null || fail 'runtime version mismatch'
grep -F 'api_version = 2' "$root/include/libpkgapply-posix/version.h" >/dev/null || fail 'API generation mismatch'
grep -F '## 3.2.3' "$root/HISTORY.md" >/dev/null || fail '3.2.3 history absent'
