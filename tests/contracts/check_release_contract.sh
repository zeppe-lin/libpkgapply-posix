...
grep -F "version: '3.2.1'" "$root/meson.build" >/dev/null || fail 'project version mismatch'
grep -F 'PROJECT_NUMBER         = 3.2.1' "$root/Doxyfile" >/dev/null || fail 'Doxygen version mismatch'
grep -F 'return "3.2.1";' "$root/src/version.cpp" >/dev/null || fail 'runtime version mismatch'
